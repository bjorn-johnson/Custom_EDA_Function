...
Custom_EDA <- function(df){
  ...
  result1 <- ggplot(gather(numeric_vars),aes(value)) +
              ...
              theme(plot.title = element_text(face="bold.italic",size=14,hjust=0.5))
  ...
  for (i in factor_vars){
    p = ggplot(factor_vars,aes(i)) +
               geom_bar(stat="count")+
               coord_flip()+
               labs(title = paste0("Categorical Variable Value Counts: ",colnames(factor_vars)[i] )) +
               theme(plot.title = element_text(face="bold.italic",size=12,hjust=0.5))
      plot(p)
    }
